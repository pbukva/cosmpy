...
ifeq ($(OS),Windows_NT)
    ...
    ifeq ($(UNAME_S),Linux)
        _FIND_CMD = find $(COSMOS_PROTO_RELATIVE_DIRS) -regextype posix-extended
    endif
    ifeq ($(UNAME_S),Darwin)
        _FIND_CMD = find -E $(COSMOS_PROTO_RELATIVE_DIRS)
    endif
endif
...
FIND_CMD = $(_FIND_CMD) -type f -name *.proto $(SOURCES_REGEX_TO_EXCLUDE:%=! -regex "%")
RELATIVE_SOURCE = $(eval RELATIVE_SOURCE := $$(shell cd $(COSMOS_SDK_DIR) && $(FIND_CMD)))$(RELATIVE_SOURCE)
UNROOTED_SOURCE = $(eval UNROOTED_SOURCE := $(foreach _,$(COSMOS_PROTO_RELATIVE_DIRS),$$(patsubst $(_)/%,%,$$(filter $(_)/%,$$(RELATIVE_SOURCE)))))$(UNROOTED_SOURCE)
SOURCE = $(eval SOURCE := $$(RELATIVE_SOURCE:%=$$(COSMOS_SDK_DIR)/%))$(SOURCE)
GENERATED = $(eval GENERATED := $$(UNROOTED_SOURCE:%.proto=$$(OUTPUT_FOLDER)/%.py))$(GENERATED)
PROTO_ROOT_DIRS = $(eval PROTO_ROOT_DIRS := $$(COSMOS_PROTO_RELATIVE_DIRS:%=$$(COSMOS_SDK_DIR)/%))$(PROTO_ROOT_DIRS)

GENERATED_DIRS = $(eval GENERATED_DIRS := $$(call unique,$(foreach _,$$(UNROOTED_SOURCE),$$(dir $(_)))))$(GENERATED_DIRS)
INIT_PY_FILES_TO_CREATE = $(eval INIT_PY_FILES_TO_CREATE :=  $$(GENERATED_DIRS:%=$$(OUTPUT_FOLDER)/%__init__.py))$(INIT_PY_FILES_TO_CREATE)

COMPILE_PROTOBUFS_COMMAND = $(eval COMPILE_PROTOBUFS_COMMAND := python -m grpc_tools.protoc $$(PROTO_ROOT_DIRS:%=--proto_path=%) --python_out=$$(OUTPUT_FOLDER) --grpc_python_out=$$(OUTPUT_FOLDER) $$(UNROOTED_SOURCE))$(COMPILE_PROTOBUFS_COMMAND)

.PHONY: generate_proto_types
generate_proto_types: $(COSMOS_SDK_DIR) $(WASMD_DIR)
	$(COMPILE_PROTOBUFS_COMMAND)

.PHONY: fetch_proto_schema_source
